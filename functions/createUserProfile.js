...
const saveProfileInfo = (credentialUsed, proxyUsed, profileId) => {
    const credentialsFilePath = path.join(__dirname, '../[UNKOYNX7]/[ONLY-CREATE]/credentials-2.txt');
...
const createUserProfiles = async () => {
    ...
    for (let i = 0; i < requestedProfiles; i++) {
        ...
        if (response.data && response.data.msg.includes('If the number of imported accounts exceeds the limit of 15')) {
            console.log(chalk.red('[ERROR]'), chalk.magenta('Já existe o máximo de perfis criados'));
